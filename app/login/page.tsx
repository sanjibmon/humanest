...
export default function Login(){
 ...
 async function submit(e:any){e.preventDefault();setBusy(true);setErr('');const r=await supabase.auth.signInWithPassword({email,password});if(r.error){setErr(r.error.message);setBusy(false);return}const aal=await supabase.auth.mfa.getAuthenticatorAssuranceLevel();if(aal.data.currentLevel==='aal2'){location.href='/hrms';return}const f=await supabase.auth.mfa.listFactors();if(f.error){setErr(f.error.message);setBusy(false);return}const verified=(f.data.totp||[]).find((x:any)=>x.status==='verified');if(!verified){location.href='/mfa-setup?next=/hrms';return}setFactor(verified.id);setMode('mfa');setBusy(false)}
